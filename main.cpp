...
bool EjectDLL(const std::string& modName, HMODULE forceHandle = nullptr) {
    ...
    // O "handle" aqui e na verdade o imageBase da regiao mapeada
    LPVOID imageBase = reinterpret_cast<LPVOID>(forceHandle);
    if (!imageBase) {
        for (auto& e : g_injected)
            if (_stricmp(e.modName.c_str(), modName.c_str()) == 0)
                { imageBase = reinterpret_cast<LPVOID>(e.handle); break; }
    }
    if (!imageBase) { AddLog("[!] Regiao mapeada nao encontrada: " + modName, COL_RED); return false; }

    HANDLE hProc = OpenProcess(PROCESS_ALL_ACCESS, FALSE, g_gtaPID);
    if (!hProc) { AddLog("[!] OpenProcess falhou.", COL_RED); return false; }

    // Libera a regiao mapeada diretamente
    ...
    AddLog("[✓] '" + modName + "' removida da memoria do jogo.", COL_GREEN);
    RefreshModules();
    return true;
}
